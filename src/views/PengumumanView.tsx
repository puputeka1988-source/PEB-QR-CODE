...
export const PengumumanView: React.FC = () => {
  ...
  return (
    <div id="view-pengumuman" className="space-y-6">
      {/* Sub Navigation Header with Breadcrumb & Back buttons */}
      <SubNavHeader
        currentTab="Pengumuman"
        activeSubTab={currentSubTab}
        onSelectSubTab={(id) => {
          if (id === 'buat-broadcast' && currentSubTab !== 'buat-broadcast') {
            handleResetForm();
          }
          setActiveSubTab('Pengumuman', id);
        }}
        badgeCounts={{
          'daftar-pengumuman': `${announcements.length} Info`,
          'buat-broadcast': editingId ? 'Edit' : '+ Baru',
          'riwayat-penerima': `${students.length} Siswa`
        }}
        extraActions={
          <div className="flex items-center gap-2">
            <div className="hidden sm:flex items-center gap-1.5 px-3 py-2 bg-slate-950/80 border border-slate-800 rounded-2xl text-xs font-semibold text-slate-300">
              <Clock className="w-3.5 h-3.5 text-emerald-400" />
              <span>Zona: <strong className="text-emerald-400 font-bold">{settings.timezone || 'WIB'}</strong></span>
            </div>
            ...
            {currentSubTab === 'daftar-pengumuman' && (
              <button
                ...
                className="px-3.5 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 rounded-2xl text-xs font-bold shadow-md shadow-emerald-500/20 flex items-center gap-1.5 transition-all cursor-pointer"
              >
      ...
      {currentSubTab === 'daftar-pengumuman' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-5 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Total Pengumuman</p>
            ...
            <div className="p-5 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Broadcast Seluruh Kelas</p>
            ...
            <div className="p-5 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Broadcast Kelas Spesifik</p>
                <p className="text-2xl font-black text-white">
            ...
            <div className="p-5 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Pesan Khusus Siswa</p>
                <p className="text-2xl font-black text-white">
          ...
          <div className="p-4 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm flex flex-col md:flex-row items-center justify-between gap-3">
            <div className="relative w-full md:w-80">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs sm:text-sm focus:outline-none focus:border-blue-500 text-white placeholder:text-slate-500"
              />
            </div>
            ...
            <div className="flex items-center gap-2 w-full md:w-auto flex-wrap">
              ...
              <select
                ...
                className="px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs sm:text-sm font-semibold text-slate-200 focus:outline-none focus:border-blue-500"
              >
          ...
          {filteredAnnouncements.length === 0 ? (
            <div className="p-12 text-center bg-slate-900 rounded-2xl border border-slate-800">
              ...
              <h3 className="text-base font-bold text-slate-200">Belum Ada Pengumuman</h3>
              ...
              <button
                ...
                className="mt-4 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold inline-flex items-center gap-1.5 shadow-md shadow-blue-500/20"
              >
                <PlusCircle className="w-4 h-4" />
              ...
              {filteredAnnouncements.map(ann => {
                ...
                return (
                  <motion.div
                    ...
                    className={`bg-slate-900 rounded-2xl border p-5 shadow-sm hover:border-slate-700 transition-all flex flex-col justify-between ${
                      ann.isPinned
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {ann.priority === 'urgent' && (
                            <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-red-600 text-white flex items-center gap-1 shadow-sm">
                      ...
                      {/* Target Indicator */}
                      <div className="mb-3 text-xs font-medium text-slate-300 flex items-center gap-1.5 flex-wrap">
                        <Bell className="w-3.5 h-3.5 text-blue-400" />
                        <span className="text-slate-400 font-semibold">Sasaran:</span>
                        {ann.targetType === 'all' && (
                          <span className="text-blue-300 font-semibold bg-blue-500/15 border border-blue-500/30 px-2 py-0.5 rounded-md">
                            Seluruh Siswa (Semua Kelas)
                          </span>
                        )}
                        {ann.targetType === 'class' && (
                          <span className="text-emerald-300 font-semibold bg-emerald-500/15 border border-emerald-500/30 px-2 py-0.5 rounded-md">
                            Kelas: {(ann.targetClasses || []).join(', ')}
                          </span>
                        )}
                        {ann.targetType === 'student' && (
                          <span className="text-amber-300 font-semibold bg-amber-500/15 border border-amber-500/30 px-2 py-0.5 rounded-md">
                      ...
                      {/* Snippet Content */}
                      <p className="text-xs sm:text-sm text-slate-200 line-clamp-3 leading-relaxed whitespace-pre-line bg-slate-950/80 p-3.5 rounded-xl border border-slate-800/80 mb-3">
                        {ann.content}
                      </p>

                      {/* Attachment Tag if any */}
                      {ann.attachmentUrl && (
                        <div className="mb-3 flex items-center gap-1.5 text-xs text-indigo-300 font-semibold">
                    ...
                    <div className="pt-3 border-t border-slate-800 flex items-center justify-between gap-2 flex-wrap">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className="p-2 rounded-xl bg-slate-800 text-slate-300 hover:text-blue-400 hover:bg-blue-600/20 border border-slate-700 hover:border-blue-500/30 transition-colors"
                        ...
                        <button
                          ...
                          className="p-2 rounded-xl bg-slate-800 text-slate-300 hover:text-red-400 hover:bg-red-600/20 border border-slate-700 hover:border-red-500/30 transition-colors"
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white rounded-xl text-xs font-bold border border-slate-700 transition-all flex items-center gap-1.5"
                        >
      ...
      {currentSubTab === 'buat-broadcast' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 space-y-6">
            <form onSubmit={handleSubmitBroadcast} className="bg-slate-900 rounded-3xl border border-slate-800 p-6 shadow-sm space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-sm font-semibold text-white focus:outline-none focus:border-blue-500 placeholder:text-slate-500"
                />
              </div>
              ...
              <div className="space-y-3 p-4 bg-slate-950/80 rounded-2xl border border-slate-800">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                  ...
                  >
                    ...
                    <p className={`text-[11px] mt-1 ${formTargetType === 'all' ? 'text-blue-100 font-medium' : 'text-slate-400'}`}>
                  ...
                  >
                    ...
                    <p className={`text-[11px] mt-1 ${formTargetType === 'class' ? 'text-blue-100 font-medium' : 'text-slate-400'}`}>
                  ...
                  >
                    ...
                    <p className={`text-[11px] mt-1 ${formTargetType === 'student' ? 'text-blue-100 font-medium' : 'text-slate-400'}`}>
                    ...
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      ...
                      <div className="relative w-48">
                        ...
                        <input
                          ...
                          className="w-full pl-8 pr-2 py-1 bg-slate-900 border border-slate-700 rounded-lg text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-blue-500"
                        />
                      </div>
                    </div>

                    {/* Student List Matrix */}
                    <div className="max-h-48 overflow-y-auto space-y-1 p-2 bg-slate-900 rounded-xl border border-slate-800">
                      {students
                        .filter(s => {
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-2 truncate">
                                {isSelected ? <CheckSquare className="w-3.5 h-3.5 text-amber-400 flex-shrink-0" /> : <Square className="w-3.5 h-3.5 text-slate-500 flex-shrink-0" />}
                                <span className="truncate">{st.name}</span>
                              </div>
                              <span className="text-[11px] text-slate-400 flex-shrink-0 bg-slate-800 px-1.5 py-0.5 rounded ml-2 border border-slate-700">
                                {st.class} • {st.nisn}
                              </span>
                            </button>
                          );
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-sm leading-relaxed text-white focus:outline-none focus:border-blue-500 placeholder:text-slate-500 font-normal"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs sm:text-sm font-semibold text-white focus:outline-none focus:border-blue-500"
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs sm:text-sm font-semibold text-white focus:outline-none focus:border-blue-500 placeholder:text-slate-500"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-slate-950/80 rounded-2xl border border-slate-800">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-4 h-4 text-blue-600 rounded border-slate-700 bg-slate-900 focus:ring-blue-500 cursor-pointer"
                  />
                  <label htmlFor="chk-pinned" className="text-xs font-semibold text-slate-200 cursor-pointer">
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
                <button
                  type="button"
                  onClick={handleResetForm}
                  className="px-4 py-2.5 text-xs sm:text-sm font-bold text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-colors cursor-pointer"
                >
                  Reset Form
                </button>
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-gradient-to-r from-blue-600 via-indigo-600 to-sky-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-xl text-xs sm:text-sm font-bold shadow-md hover:shadow-lg transition-all flex items-center gap-2 cursor-pointer"
                >
          ...
          <div className="lg:col-span-5 space-y-4">
            <div className="p-5 bg-slate-900 rounded-3xl border border-slate-800 shadow-sm">
              ...
              <div className="bg-slate-950 rounded-2xl border border-slate-800 overflow-hidden shadow-lg">
                ...
                <div className="p-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
                  ...
                  <h4 className="text-sm sm:text-base font-bold mt-1.5 leading-snug line-clamp-2 text-white">
                    {formTitle || 'Judul Pengumuman Akan Ditampilkan di Sini'}
                  </h4>
                  <p className="text-[11px] text-blue-100 mt-1 flex items-center gap-1 font-medium">
                ...
                <div className="p-4 space-y-3 bg-slate-950">
                  <div className="flex items-center justify-between text-[11px] text-slate-400">
                    <span>Oleh: <strong className="text-slate-200 font-semibold">{formAuthorName || 'Administrator'}</strong></span>
                    <span>Hari Ini • Baru saja</span>
                  </div>

                  <p className="text-xs sm:text-sm text-slate-200 leading-relaxed whitespace-pre-line bg-slate-900 p-3.5 rounded-xl border border-slate-800 min-h-[80px]">
                    {formContent || 'Isi teks pengumuman yang diketik akan muncul di sini...'}
                  </p>

                  {formAttachmentUrl && (
                    <div className="p-2.5 bg-indigo-950/60 border border-indigo-800/60 rounded-xl text-xs text-indigo-300 font-semibold flex items-center justify-between">
                  ...
                  <div className="pt-2 flex justify-end">
                    <div className="px-3.5 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 shadow-sm">
      ...
      {currentSubTab === 'riwayat-penerima' && (
        <div className="space-y-6">
          ...
          <div className="p-5 bg-slate-900 rounded-3xl border border-slate-800 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="space-y-1 w-full md:w-auto">
              ...
              <select
                ...
                className="w-full md:w-96 px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-sm font-semibold text-white focus:outline-none focus:border-blue-500"
              >
            ...
            {activeSelectedAnn && (
              <div className="flex items-center gap-3 flex-wrap">
                <span className="text-xs text-slate-300 font-medium">
                  Sasaran: <strong className="text-white font-bold">{activeSelectedAnn.targetType === 'all' ? 'Semua Kelas' : activeSelectedAnn.targetType === 'class' ? (activeSelectedAnn.targetClasses || []).join(', ') : 'Personal'}</strong>
                </span>
          ...
          {activeSelectedAnn && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-5 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm">
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Total Target Siswa</p>
                <p className="text-2xl font-black text-white mt-1">
                  {recipientData.targetStudents.length} <span className="text-xs font-normal text-slate-400">Siswa</span>
                </p>
              </div>
              ...
              <div className="p-5 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm">
                ...
                <p className="text-2xl font-black text-emerald-400 mt-1">
                  {recipientData.readCount} <span className="text-xs font-normal text-slate-400">
              ...
              <div className="p-5 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm">
                ...
                <p className="text-2xl font-black text-amber-400 mt-1">
                  {recipientData.unreadCount} <span className="text-xs font-normal text-slate-400">
          ...
          <div className="p-4 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 bg-slate-950 border border-slate-700 rounded-xl text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-blue-500"
              />
            </div>
          </div>
          ...
          <div className="bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-950 text-slate-400 uppercase tracking-wider font-bold text-[11px] border-b border-slate-800">
                  <tr>
                    <th className="p-4">No</th>
                    <th className="p-4">Nama Siswa</th>
                    <th className="p-4">NISN</th>
                    <th className="p-4">Kelas</th>
                    <th className="p-4">Status Pembaca</th>
                    <th className="p-4">Waktu Membaca</th>
                  </tr>
                </thead>
                  ...
                  {filteredRecipientList.length === 0 ? (
                    ...
                    filteredRecipientList.map((item, idx) => (
                      <tr key={item.student.id} className="hover:bg-slate-800/50 transition-colors">
                        ...
                        <td className="p-4 font-mono text-slate-300">{item.student.nisn}</td>
                        <td className="p-4 font-semibold text-slate-200">{item.student.class}</td>
                        <td className="p-4">
                          {item.isRead ? (
                        ...
                        <td className="p-4 text-slate-300">
                          {item.readAt ? (
                            ...
                            <span className="text-slate-500 italic">Belum dibuka</span>
                          )}
                        </td>
                      </tr>
                    ))
